...
public extension Array {
    ...
    func split(partSize: Int) -> [[Element]] {
        guard partSize > 0
            ...
            error("[Array] illegal argument. partSize:[\(partSize)]")
            return [self]
        }
    ...
    func transform<T>(key field: KeyPath<Element, T>) -> [T:Element] {
        reduce(into: [ : ]) { map, element in
            map[element[keyPath: field]] = element
        }
    }
    
    func transform<K,V>(key field: KeyPath<Element, K>, _ transform: (Element) -> V?) -> [K:V] {
        reduce(into: [ : ]) { map, element in
            if let transformed: V = transform(element) {
                map[element[keyPath: field]] = transformed
            }
        }
    }
...
public extension Array where Element : Equatable {
    ...
    mutating func addUnique(among array: [Element]) {
        array.forEach { addUnique($0) }
    }

    mutating func addUnique(_ element: Element) {
        if element.isAbsent(among: self) {
            append(element)
        }
    }
...
public extension Equatable {
    ...
    func isAbsent(among elements: Array<Self>) -> Bool {
        !( elements.contains(self) )
    }

    func isOne(of elements: Array<Self>) -> Bool {
        elements.contains(self)
    }

}

public func +=<E>(array: inout Array<E>, _ element: E) {
    array.append(element)
}

public func +=<E>(set: inout Set<E>, _ element: E) {
    set.insert(element)
}

public extension Comparable {

    func isAbsent(among elements: Range<Self>) -> Bool {
        elements.contains(self) == false
    }
    
    func isAbsent(among elements: ClosedRange<Self>) -> Bool {
        elements.contains(self) == false
    }

    func isOne(of elements: Range<Self>) -> Bool {
        elements.contains(self)
    }
    
    func isOne(of elements: ClosedRange<Self>) -> Bool {
        elements.contains(self)
    }
    
    func `is`(in range: Range<Self>) -> Bool {
        range.contains(self)
    }
    
    func `is`(in range: ClosedRange<Self>) -> Bool {
        range.contains(self)
    }
    
    func `is`(outOf range: Range<Self>) -> Bool {
        range.contains(self) == false
    }
    
    func `is`(outOf range: ClosedRange<Self>) -> Bool {
        range.contains(self) == false
    }
...
public extension Array {
    ...
    subscript(safe index: Int) -> Element? {
        ...
        set(new) { if (index > -1) && (index < count) { update(at: index, with: new) } }
    }
    
    private mutating func update(at index: Int, with new: Element?) {
        unwrap(new) { self[index] = $0 }
    }
...
private func error(_ record: String) {
    log(error: record)
}
