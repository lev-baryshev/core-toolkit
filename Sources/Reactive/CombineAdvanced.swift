...
public extension Combine.Publishers {
    ...
    static func reborn<T, F:Error>(
        _ constructPublisher:   @escaping () -> AnyPublisher<T, F>?,
...
public extension Combine.Empty {
    ...
    init(_ output: Output.Type, _ failure: Failure.Type) {
        self.init(outputType: output, failureType: failure)
    }
...
public extension AnyCancellable {
    ...
    func store(_ store: (AnyCancellable) -> Void) {
        store(self)
    }
