...
public extension Combine.Publisher {
    ...
    func consumeFailure(_ react: @escaping (Failure) -> Void = { _ in }) -> some Publisher<Output, Never> {
        self.catch { react($0); return Empty(outputType: Output.self, failureType: Never.self) }
    }
...
public extension Combine.Publisher where Failure == Never {
    ...
    func abstract() -> Downstream<Output> { eraseToAnyPublisher() }
...
-> some Publisher<Output, Failure> {
    constructRecover() ?? Empty(outputType: Output.self, failureType: Failure.self).anyPublisher
}
