...
public extension Json {
    ...
    func dates(_ format: DatesFormat) -> Json {
        switch format {
        case .iso8601:
            encoder.dateEncodingStrategy = .iso8601
            decoder.dateDecodingStrategy = .iso8601
        case .custom(let template, let seconds):
            // fixme: test timezone seconds > 0; should we invert seconds when deserializing
            let en: Locale? = .init(identifier: "en")
            encoder.dateEncodingStrategy = .formatted(.with(template, timezoneSeconds: seconds, locale: en))
            decoder.dateDecodingStrategy = .formatted(.with(template, timezoneSeconds: seconds, locale: en))
        }
        return self
    }
    ...
    enum DatesFormat {
        case iso8601
        case custom(_ template: String, _ timezoneSeconds: Int = TimeZoneId.utc.seconds)
    }
...
public extension Json {
    ...
    func serialize<T:Encodable>(_ value: T) throws -> JsonBinary {
        try encoder.encode(value)
    }
    
    func serialize<T:Encodable>(_ value: T) throws -> JsonString? {
        try encoder.encode(value).string
    }
    ...
    func serialize<T:Encodable>(_ value: T) throws -> [String : Any] {
        ...
        guard let serialized = try JSONSerialization.jsonObject(with: json, options: [ ]) as? [String : Any]
            ...
            throw Exception("[Json] unable to serialize: \(T.self)")
        }
        return serialized
    }
